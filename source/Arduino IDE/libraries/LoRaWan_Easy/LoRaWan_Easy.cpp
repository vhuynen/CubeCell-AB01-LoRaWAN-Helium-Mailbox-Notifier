...
#if(AT_SUPPORT)
#error "The LoRaWanEasy library is configured for LORAWAN_AT_SUPPORT=OFF. Please disable."
#endif
...
void LoRaWanEasy::begin(DeviceClass_t loraWanClass,LoRaMacRegion_t region)
{
...
bool LoRaWanEasy::joinOTAA(uint8_t* appEui, uint8_t* appKey)
{
    return joinOTAA(appEui, appKey, generateDevEUIByChipID());
}

bool LoRaWanEasy::joinOTAA(uint8_t* appEui, uint8_t* appKey, uint8_t* devEui)
{
	MlmeReq_t mlmeReq;
...
bool LoRaWanEasy::joinABP(uint8_t* nwkSKey, uint8_t* appSKey, uint32_t devAddr)
{
	MibRequestConfirm_t mibReq;
...
bool LoRaWanEasy::send(uint8_t datalen, uint8_t port, bool confirmed)
{
	MibRequestConfirm_t mibReq;
	mibReq.Type = MIB_DEVICE_CLASS;
	LoRaMacMibGetRequestConfirm( &mibReq );
	...
	{
		printf("LoRaWan_Easy.send: Bad LoRaMacMcpsRequest status, couldn't send %d", status);
		return false;
	}
...
uint8_t* LoRaWanEasy::generateDevEUIByChipID()
{
